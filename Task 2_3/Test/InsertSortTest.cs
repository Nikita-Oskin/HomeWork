...
namespace Block2_3.Tests
{
    ...
    public class InsertSortTest
    {
        ...
        public void InsertSortTest1()
        {
            ...
            insert.Items.AddRange(iarray1);

            insert.Sort();

            for (int i = 0; i < iarray1.Count(); i++)
            {
                Assert.AreEqual(sortediarray1[i], insert.Items[i]);
            }
        }
        ...
        {
            var insert = new InsertSort<int>();
            insert.Items.AddRange(iarray2);

            insert.Sort();

            for (int i = 0; i < iarray2.Count(); i++)
            {
                Assert.AreEqual(sortediarray2[i], insert.Items[i]);
            }
        }
        ...
        {
            var insert = new InsertSort<int>();
            insert.Items.AddRange(iarray3);

            insert.Sort();

            for (int i = 0; i < iarray3.Count(); i++)
            {
                Assert.AreEqual(sortediarray3[i], insert.Items[i]);
            }
        }
        ...
        {
            var insert = new InsertSort<int>();
            insert.Items.AddRange(iarray4);

            insert.Sort();

            for (int i = 0; i < iarray4.Count(); i++)
            {
                Assert.AreEqual(sortediarray4[i], insert.Items[i]);
            }
        }

        // 4 теста для типа данных string
        [TestMethod]
        public void InsertSortTest5()
        {
            var insert = new InsertSort<string>();
            insert.Items.AddRange(sarray1);

            insert.Sort();

            for (int i = 0; i < sarray1.Count(); i++)
            {
                Assert.AreEqual(sortedsarray1[i], insert.Items[i]);
            }
        }

        [TestMethod]
        public void InsertSortTest6()
        {
            var insert = new InsertSort<string>();
            insert.Items.AddRange(sarray2);

            insert.Sort();

            for (int i = 0; i < sarray2.Count(); i++)
            {
                Assert.AreEqual(sortedsarray2[i], insert.Items[i]);
            }
        }

        [TestMethod]
        public void InsertSortTest7()
        {
            var insert = new InsertSort<string>();
            insert.Items.AddRange(sarray3);

            insert.Sort();

            for (int i = 0; i < sarray3.Count(); i++)
            {
                Assert.AreEqual(sortedsarray3[i], insert.Items[i]);
            }
        }

        [TestMethod]
        public void InsertSortTest8()
        {
            var insert = new InsertSort<string>();
            insert.Items.AddRange(sarray4);

            insert.Sort();

            for (int i = 0; i < sarray4.Count(); i++)
            {
                Assert.AreEqual(sortedsarray4[i], insert.Items[i]);
            }
        }
